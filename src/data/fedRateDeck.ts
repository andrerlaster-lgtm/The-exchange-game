...
const rateCard = (
  ...
  return { ...mk(title, story, `${parts.join(' · ')}.`, eff, stance, insight), rateBp, rateShock: true };
};

/** A card with its own price effect that also nudges the Bank Rate. */
const withRate = (card: Card, rateBp: number): Card => ({
  ...card,
  effect: `${card.effect.replace(/\.$/, '')} · Bank Rate ${upDown(rateBp)} ${moveSize(rateBp)}.`,
  rateBp,
});

export const FED_CARDS: Card[] = [
  rateCard('Jumbo Hike', 'Inflation runs hot and the Fed moves a full point.', 100, 'hawkish', 'A rare, outsized rise: banks gain sharply while property and high-risk growth companies are hit hard, and every loan gets much more expensive.'),
  rateCard('Emergency Cut', 'A shock to the system forces the Fed to act fast.', -100, 'dovish', 'A rare, outsized cut: property and high-risk growth companies rally hard, bank margins are squeezed, and borrowing gets much cheaper.'),
  ...
  withRate(mk('Inflation Warning', 'The Fed signals policy may stay tight.', 'Consumer DOWN 5% (500 bp) · High-Risk DOWN 5% (500 bp).', { k: 'multi', m: [{ sec: 'consumer', bp: -500 }, { risk: 'High', bp: -500 }] }, 'hawkish', 'Persistent inflation pressures consumer spending and speculative companies.'), 25),
  ...
  withRate(mk('Bond Yields Rise', 'Safer income investments get more attractive.', 'Low-Risk stocks UP 5% (500 bp) · High-Risk stocks DOWN 5% (500 bp).', { k: 'multi', m: [{ risk: 'Low', bp: 500 }, { risk: 'High', bp: -500 }] }, 'hawkish', 'Investors rotate toward dependable income and away from speculative companies.'), 25),
  withRate(mk('Cut Hopes Rally', 'Investors expect future cuts and chase growth.', 'Technology UP 5% (500 bp) · Consumer UP 5% (500 bp).', { k: 'multi', m: [{ sec: 'tech', bp: 500 }, { sec: 'consumer', bp: 500 }] }, 'dovish', 'Expected rate relief increases demand for growth and consumer companies.'), -25),
];
