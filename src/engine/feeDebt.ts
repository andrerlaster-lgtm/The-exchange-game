import { BANK_RATE_START_BP, FEE_DEBT_INSTALLMENT, FEE_DEBT_MIN_INTEREST, FEE_DEBT_SPREAD_BP } from '../data';
...
/** Add one turn of compounding interest at `ratePct` (the Bank Rate plus the
    fee spread — see feeDebtRatePct), rounded to $10 with a $100 minimum.
...
export function accrueFeeDebt(player: Player, ratePct = (BANK_RATE_START_BP + FEE_DEBT_SPREAD_BP) / 100): number {
  const balance = feeDebtBalance(player);
  if (balance <= 0) return 0;
  const rounded = Math.round(balance * ratePct / 100 / 10) * 10;
